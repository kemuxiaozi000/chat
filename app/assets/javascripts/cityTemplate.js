// 城市选择框显示
var pinyinOrder = new Object();
pinyinOrder.cssParents = function () { //城市显示
    // pinyinOrder.top = placeThis.offset().top - 163;  //城市选择框  top位置
    // pinyinOrder.left = placeThis.offset().left + 8;   //城市选择框  left位置
    pinyinOrder.top = placeThis.offset().top - placeThis.parent().parent().offset().top + placeThis.height() + 16; //城市选择框  top位置
    pinyinOrder.left = placeThis.offset().left - placeThis.parent().parent().offset().left;
    $(pinyinOrder.id).css({
        "top": pinyinOrder.top + "px",
        "left": pinyinOrder.left + "px",
        "width": pinyinOrder.width + "px",
        "height": pinyinOrder.height + "px"
    });
}
pinyinOrder.place = function (e) {
    e.click(function () {
        placeThis = $(this);
        //城市显示
        pinyinOrder.cssParents();
        $(pinyinOrder.id).show();
        return false;
    })
}
pinyinOrder.destination = function (e) {
    e.click(function () {
        placeThis = $(this);
        //城市显示
        pinyinOrder.cssParents();
...
/* 城市HTML模板 */
pinyinOrder._template = [
    '<div class="screen">',
    '<a href="javascript:void(0)">A-F</a>',
    '<a href="javascript:void(0)">G-L</a>',
    '<a href="javascript:void(0)">M-S</a>',
    '<a href="javascript:void(0)">T-Z</a>',
    '</div>',
    '<div class="city_pos">',
...
// pinyinOrder.allMember = ['北京|beijing|bj', '上海|shanghai|sh', '重庆|chongqing|cq', '深圳|shenzhen|sz', '广州|guangzhou|gz', '杭州|hangzhou|hz',
...
// 热门城市
pinyinOrder.Hot = function (cityA) {
    var ckey, odda, sortKey, str, odda = [],
        abc = [],
        key, regEx = pinyinOrder.regEx,
        oCity = pinyinOrder.oCity,
        len, leni;
    for (key in oCity) {
        sortKey = [];
        ...
        for (var j = 0, k = sortKey.length; j < k; j++) {
            ...
            for (var i = 0, n = oCity[key][sortKey[j]].length; i < n; i++) {
                ...
                // if (key == 'hot') {
                //     $(pinyinOrder.id).find(pinyinOrder.Children).eq(0).append('<a href="javascript:void(0)">' + oCity[key][sortKey[j]][i] + '</a>');
                //     odda.push(str);
                // } else {
                str = '<a href="javascript:void(0)">' + oCity[key][sortKey[j]][i] + '</a>';
                pinyinOrder.arrRepeat(abc, sortKey, j); //获取字母
                odda.push(str);
                len = n;
                leni = i;
                // }
            ...
            pinyinOrder.cityPinyin(leni, len, key, abc, odda);
        }
    }
}

// 按拼音排序
pinyinOrder.cityPinyin = function (leni, len, key, abc, odda) {
    ...
    if (leni != undefined) {
        if (len - 1 == leni) {
            ...
            // $(pinyinOrder.id).find(pinyinOrder.Children).eq(one).append('<div class="Letter">'
            $(pinyinOrder.id).find(pinyinOrder.Children).eq(one - 1).append('<div class="Letter">' +
                '<span>' + abc[0] + '</span>' +
                '<div>' +
                odda.join('') +
                '</div>' +
...
// 数组去重
pinyinOrder.arrRepeat = function (abc, sortKey, j) {
    var nab = sortKey[j];
    for (var i in abc) {
        if (abc[i] == nab) {
            return nab = 1;
...
// 城市切换
pinyinOrder.payment = function ($this) {
    var ind = $this.index();
...
// 给input赋值
pinyinOrder.cityClick = function ($this) {
    $(".city_a_le1 a").click(function () {
        var a_city = $(this).text(); //当前选择的城市
        $(pinyinOrder.id).hide(); //隐藏城市选择框
        ...
        // TODO 判断浏览器
        if (app.$data) {
            app.$data.query.brandName = a_city
        }
