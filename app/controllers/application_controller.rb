...
class ApplicationController < ActionController::Base
  ...
  def session_save
    p params[:u_id]
    $logdata.debug("请求controller ： #{self.class.controller_path}")
    $logdata.debug("请求action： #{self.action_name}")
    $logdata.debug("请求参数： #{params.inspect}")
    if session[:logmsg].nil?
      session[:logmsg] = LogMsg.instance
    end
    if params[:token].present?
      session[:current_user_id] = params[:u_id]
      session[:token] = params[:token]
      # $logdata.info(session[:logmsg].msg_get('I_MSG0001', session[:current_user_id], 'session_save'))
      begin
        userDetail = UserDetail.find_by_sub_u_id(params[:u_id])
        # userDetail = get_user_detail_by_subuid(params[:u_id])
        if userDetail.nil?
          # $logdata.error(session[:logmsg].msg_get('E_MSG0002', session[:current_user_id]))
          session_clear
      ...
      session[:role] = userDetail.u_level
      session[:last_time] = Time.now
      #iframe
      if params[:iframe].present? && params[:iframe]= "true"
        return
      end

      #用户编辑后跳转
      if !params[:view].nil? && params[:view] == "my_profile"
        session[:tab] = 3 if params[:tab] == "3"
        # $logdata.info(session[:logmsg].msg_get('I_MSG0003', session[:current_user_id]))
        redirect_to("#{ENV['ACCESS_PASS']}/repair_case/my_profile")
        return
      end

      if params[:type].blank?
        browseInfo = UserBrowseInfo.find_by_current_user_id(session[:current_user_id])
        if browseInfo.present?
          # common user
          if browseInfo[:last_page] == "detail" && browseInfo[:repaircase_id].present?
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/repair_case/detail/#{browseInfo[:repaircase_id]}?")
            return
          elsif browseInfo[:last_page] == "search"
            redirect_to("#{ENV['ACCESS_PASS']}/repair_case/search_case")
            return
          elsif browseInfo[:last_page] == "post_case"
            redirect_to("#{ENV['ACCESS_PASS']}/repair_case/post_case")
            return
          elsif browseInfo[:last_page] == "notice"
            redirect_to("#{ENV['ACCESS_PASS']}/notice")
            return
          elsif browseInfo[:last_page] == "my_profile"
            redirect_to("#{ENV['ACCESS_PASS']}/repair_case/my_profile")
            return
          elsif browseInfo[:last_page] == "personal" && browseInfo[:user_id].present?
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/personal?u_id=#{browseInfo[:user_id]}")
            return
          elsif browseInfo[:last_page] == "notice"
            redirect_to("#{ENV['ACCESS_PASS']}/notice")
            return
          #specialist
          elsif browseInfo[:last_page] == "specialist_view"
            redirect_to("#{ENV['ACCESS_PASS']}/specialist/view")
            return
          elsif browseInfo[:last_page] == "specialist_detail" && browseInfo[:repaircase_id].present?
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/specialist/repaircase_detail/#{browseInfo[:repaircase_id]}")
            return
          elsif browseInfo[:last_page] == "specialist_history_detail" && browseInfo[:repaircase_id].present?
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/specialist/repaircase_history/#{browseInfo[:repaircase_id]}")
            return
          elsif browseInfo[:last_page] == "specialist_list"
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/specialist/repaircase_detail")
            return
          elsif browseInfo[:last_page] == "specialist_history_list"
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/specialist/repaircase_history")
            return
          # admin
          elsif browseInfo[:last_page] == "admin_view"
            redirect_to("#{ENV['ACCESS_PASS']}/admin/view")
            return
          elsif browseInfo[:last_page] == "admin_management_db_detail" && browseInfo[:repaircase_id].present?
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/admin/management_db_detail/#{browseInfo[:repaircase_id]}")
            return
          elsif browseInfo[:last_page] == "admin_db"
            redirect_to("#{ENV['ACCESS_PASS']}/admin/management_db")
            return
          elsif browseInfo[:last_page] == "admin_information"
            redirect_to("#{ENV['ACCESS_PASS']}/admin/information")
            return
          elsif browseInfo[:last_page] == "admin_specialist"
            redirect_to("#{ENV['ACCESS_PASS']}/admin/management_specialist_group")
            return
          elsif browseInfo[:last_page] == "admin_tipoff_case"
            redirect_to("#{ENV['ACCESS_PASS']}/admin/tipoff_case")
            return
          elsif browseInfo[:last_page] == "admin_tipoff_comment"
            redirect_to("#{ENV['ACCESS_PASS']}/admin/tipoff_comment")
            return
          elsif browseInfo[:last_page] == "admin_tipoff_parts"
            redirect_to("#{ENV['ACCESS_PASS']}/admin/tipoff_parts")
            return
          elsif browseInfo[:last_page] == "admin_content" && browseInfo[:repaircase_id].present?
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/admin/repaircase_detail/#{browseInfo[:repaircase_id]}")
            return
          elsif browseInfo[:last_page] == "admin_repaircase_list"
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/admin/repaircase_detail")
            return
          elsif browseInfo[:last_page] == "admin_repaircase_publish" && browseInfo[:repaircase_id].present?
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/admin/repaircase_publish/#{browseInfo[:repaircase_id]}")
            return
          elsif browseInfo[:last_page] == "admin_repaircase_publish_list"
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/admin/repaircase_publish")
            return
          elsif browseInfo[:last_page] == "admin_repaircase_reject"
            session[:forbiddenReturn] = true
            redirect_to("#{ENV['ACCESS_PASS']}/admin/repaircase_reject")
            return
          end
        end

        if userDetail.u_level == '0'
          # $logdata.info(session[:logmsg].msg_get('I_MSG0009', session[:current_user_id]))
          redirect_to("#{ENV['ACCESS_PASS']}/admin/view")
          return
        elsif userDetail.u_level == '1' || userDetail.u_level == '2'
          # $logdata.info(session[:logmsg].msg_get('I_MSG0004', session[:current_user_id]))
          redirect_to("#{ENV['ACCESS_PASS']}/specialist/view")
          return
        else
          # $logdata.info(session[:logmsg].msg_get('I_MSG0003', session[:current_user_id]))
        ...
        diagnosisData1 = Hash.new
        diagnosisData1[:brand] = params[:brand] if params[:brand].present?
        # 车名不显示
        #diagnosisData1[:car_type] = params[:car_type] if params[:car_type].present?
        diagnosisData1[:mileage] = params[:mileage] if params[:mileage].present?
        diagnosisData1[:model_year] = params[:model_year] if params[:model_year].present?
        diagnosisData1[:code] = params[:code].split("#") if params[:code].present?
        redirect_to :controller => "/repair_case/search_case", :action => "index", :diagnosisData => diagnosisData1
        return
      end

    elsif params[:type].present?
      session[:isLaunch] = true
      p "type.present? type=#{params[:type]}"
      #未登录
      #诊断仪登录
      diagnosisData = Hash.new
      diagnosisData[:brand] = params[:brand] if params[:brand].present?
      # 车名不显示
      #diagnosisData[:car_type] = params[:car_type] if params[:car_type].present?
      diagnosisData[:mileage] = params[:mileage] if params[:mileage].present?
      diagnosisData[:model_year] = params[:model_year] if params[:model_year].present?
      diagnosisData[:code] = params[:code].split("#") if params[:code].present?
      redirect_to :controller => "/repair_case/search_case", :action => "index", :diagnosisData => diagnosisData
    end
  end
