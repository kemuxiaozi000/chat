...
resource "aws_s3_bucket" "aws_eb_s3_staging" {
  bucket = "${local.staging_eb_bucket_name}"
...
resource "aws_elastic_beanstalk_environment" "aws_eb_lupin_app_staging_env" {
  name         = "${var.app_name}-${local.staging_environment}"
  application  = "${aws_elastic_beanstalk_application.aws_eb_lupin_app.name}"
  cname_prefix = "${var.app_name}-${var.account_id}-${local.staging_environment}"
  tier         = "WebServer"
  ...
  setting {
    namespace = "aws:autoscaling:launchconfiguration"
    name      = "InstanceType"
    value     = "${var.staging_web_instance_type}"
  }
  setting {
    namespace = "aws:autoscaling:launchconfiguration"
    name      = "EC2KeyName"
  ...
  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "DB_NAME"
    value     = "${var.app_name}-${local.staging_environment}"
  }
  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "RUNNING_MODE"
  ...
  setting {
    ...
    value     = "${local.staging_environment}"
  }
  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "REGION"
